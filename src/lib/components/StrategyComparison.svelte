...
<div class="space-y-6">
		...
		<div class="bg-green-50 border-l-4 border-green-500 p-4 rounded-r-lg">
			<div class="flex">
				...
				<div class="ml-3">
					...
					<p class="mt-1 text-sm text-green-700">{recommended.reason}</p>
				</div>
			</div>
		</div>
	{/if}
	...
	<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-{strategies.length > 2 ? '4' : '2'} gap-4">
		{#each strategies as strategy}
			...
			<div
				class="bg-white rounded-lg shadow-md p-6 border-2 {isRecommended
					? 'border-green-500'
				...
				<div class="space-y-4">
					...
					<div>
						...
						{#if isBestTime && strategy.key !== 'current'}
							<span class="text-xs text-green-600 font-medium">Fastest</span>
						{/if}
					</div>

					<!-- Total Months -->
					<div>
						<p class="text-sm text-gray-600">Time to Payoff</p>
						<p class="text-lg font-semibold text-gray-900">
					...
					<div>
						...
						{#if isBestInterest && strategy.key !== 'current'}
							<span class="text-xs text-green-600 font-medium">Lowest Interest</span>
						{/if}
						{#if comparison.current && strategy.key !== 'current'}
	...
	<div class="bg-white rounded-lg shadow-md overflow-hidden">
		<div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
			<h3 class="text-lg font-semibold text-gray-900">Side-by-Side Comparison</h3>
		</div>
		<div class="overflow-x-auto">
			<table class="min-w-full divide-y divide-gray-200">
				...
				<tbody class="bg-white divide-y divide-gray-200">
					{#each strategies as strategy}
						{@const isRecommended = recommended?.strategy === strategy.key}
						<tr class="{isRecommended ? 'bg-green-50' : 'hover:bg-gray-50'} transition-colors">
